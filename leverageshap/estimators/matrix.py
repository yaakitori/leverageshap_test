...
class MatrixEstimator:
    def __init__(self, model, baseline, explicand, num_samples, paired_sampling=False, bernoulli_sampling=False):
        self.model = model
        ...
        self.sample_weight = lambda s : 1 / scipy.special.comb(self.n-1,s) + 1/scipy.special.comb(self.n-1,s-1)
        self.kernel_weights = []
    ...
    def sample_with_replacement(self):
        ...
        for idx, s in enumerate(sampled_sizes):
            ...
            weight = 1/(prob_sizes[s-1] * scipy.special.comb(self.n, s))
    ...
    def compute(self):
        ...
        S_short = np.zeros((self.n, SZ_binary.shape[0]))
        for i in range(SZ_binary.shape[0]):
            size = int(np.sum(SZ_binary[i]))
...
def matrix_shap(baseline, explicand, model, num_samples):
    estimator = MatrixEstimator(model, baseline, explicand, num_samples, paired_sampling=True, bernoulli_sampling=True)
    return estimator.compute()

def matrix_shap_wo_bernoulli(baseline, explicand, model, num_samples):
    estimator = MatrixEstimator(model, baseline, explicand, num_samples, paired_sampling=True, bernoulli_sampling=False)
    return estimator.compute()

def matrix_shap_wo_bernoulli_paired(baseline, explicand, model, num_samples):
    estimator = MatrixEstimator(model, baseline, explicand, num_samples, paired_sampling=False, bernoulli_sampling=False)
    return estimator.compute()
